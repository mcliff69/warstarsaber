/*
     СУПЕР-ПУПЕР-МЕГА-ПЕГА-КРУТОЙ СВЕТОВОЙ МЕЧ НА АРДУИНЕ И АДРЕСНЫХ СВЕТОДИОДАХ!
   ...
   УПРАВЛЕНИЕ:
      ...
      3. Если питание включено, но меч не включен, то через 10 минут воспроизводится напоминание, что нужно выключить питание. 
        Файл "09/003.mp3" на карте.
      ...
      В режиме регулировки громкости.
...
//#endif
  ...
  // режим Регулировка громкости
    ...
    if (!btn_on && btn_time == now && nowVolume < 10) {
        nowVolume++;                           
        mp3.setVolume(nowVolume * 3);
    ...
    } else if (!btn_on2 && btn_time2 == now && nowVolume > 0) {
        nowVolume--;                           
        mp3.setVolume(nowVolume * 3);
